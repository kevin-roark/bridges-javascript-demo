...
$(function() {
  ...
  socket.on('got you a color', function(color) {
    ...
    //bigTween.easing(TWEEN.Easing.Circular.Out);
    bigTween.start();
  ...
  function addBox(color) {
    ...
    var targetScale = {value: 1};
    var updateMeshScale = function() { box.scale.set(scale.value, scale.value, scale.value); };
    updateMeshScale();

    var scaleTween = new TWEEN.Tween(scale).to(targetScale, 500);
    scaleTween.onUpdate(updateMeshScale);
    scaleTween.start();
